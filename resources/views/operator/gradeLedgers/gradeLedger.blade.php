...
<div class="page-title">
    <h3>Ledger Kelas</h3>
    <p class="text-subtitle text-muted">Ledger kelas <strong>{{ $subGrade->sub_grade ?? null }}</strong></p>
</div>
<hr>

<section class="section">

    <form action="/operator/gradeLedger" method="get" class="mb-2">
        <select name="subGrade" class="form-select form-select-sm w-auto" onchange="this.form.submit()">
            @foreach ($subGrades as $subGrade)
            <option value="{{ $subGrade->id }}" {{ selected($subGrade->id == $selected)
                }}>{{ $subGrade->sub_grade }}
            </option>
            @endforeach
        </select>
