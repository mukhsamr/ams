...
<section class="section">
    <div class="row g-1 mb-2">
        <div class="col-12 col-md-6 order-2 order-md-0">
            <form action="/operator/competences" method="get" class="row g-1" id="search-competences">
                <div class="col-6">
                    <select name="subject" class="form-select form-select-sm" id="subject">
                        @foreach($subjects as $subject)
                        <option value="{{ $subject->id }}" {{ selected($selected['subject']==$subject->id) }}>{{
                            $subject->subject }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col">
                    <select name="grade" class="form-select form-select-sm" id="grade">
                        <option hidden></option>
                        @foreach($grades as $grade)
                        <option value="{{ $grade->id }}" {{ selected($selected['grade']==$grade->id) }}>{{ $grade->grade }}</option>
                        @endforeach
                    </select>
                </div>
            </form>
            ...
            <table class="table table-bordered mb-0">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="text-center">{{ $competence->grade }}</td>
                        <td class="text-center">
                            <a href="/operator/competences/edit/{{ $competence->id }}" class="btn btn-sm btn-info load-modal" data-target="#edit"><i data-feather="edit"></i></a>
                        </td>
                        <td class="text-center">
...
@push('scripts')
<script src="{{ asset('js/operator/competence.js') }}"></script>
@endpush
